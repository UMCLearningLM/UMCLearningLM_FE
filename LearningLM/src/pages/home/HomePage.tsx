...
const categories = [
  { icon: '자', title: '자료조사', description: '튜토리얼 · 공개 흐름' },
  { icon: '문', title: '문서 요약', description: '튜토리얼 · 공개 흐름' },
  { icon: '글', title: '글쓰기', description: '튜토리얼 · 공개 흐름' },
  { icon: '반', title: '반복 작업 정리', description: '튜토리얼 · 공개 흐름' },
  { icon: '검', title: '결과물 검토', description: '튜토리얼 · 공개 흐름' },
  { icon: 'A', title: 'AI 툴 활용', description: '튜토리얼 · 공개 흐름' },
]
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <PageContainer className="space-y-10">
        <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm lg:p-10">
          <div className="grid gap-10 lg:grid-cols-[1fr_520px] lg:items-center">
            <div>
              ...
              <div className="mt-6 flex h-12 max-w-md items-center gap-3 rounded-full border border-slate-200 bg-white px-4 text-slate-400 shadow-sm">
                <Search size={18} />
                <input
                  type="text"
        ...
        <Section title="추천 공식 튜토리얼" actionLabel="전체 보기">
              ...
              >
                ...
                <CardBody>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge variant={tutorial.levelVariant}>{tutorial.level}</Badge>
                  </div>
                ...
                <CardFooter>
                  <div className="flex items-center gap-3 text-xs text-slate-400">
                    <span className="inline-flex items-center gap-1">
                      <Bookmark size={14} /> {tutorial.bookmarks}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      <Clock size={14} /> {tutorial.minutes}분
                    </span>
                  </div>
        ...
        <Section title="인기 공개 워크플로우" actionLabel="라이브러리">
          <div className="grid gap-5 md:grid-cols-3">
              ...
              >
                <CardBody>
                  ...
                  <div className="mt-5 flex items-start justify-between gap-3">
                    ...
                    <Badge variant={workflow.levelVariant}>{workflow.level}</Badge>
                  </div>
                ...
                <CardFooter>
                  <div className="flex items-center gap-3 text-xs text-slate-400">
                    <span className="inline-flex items-center gap-1">
                      <Bookmark size={14} /> {workflow.saves}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      <Copy size={14} /> 복사 {workflow.copies}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      <MessageCircle size={14} /> {workflow.comments}
                    </span>
                  </div>

                  <Button variant="link" rightIcon={<ArrowRight size={15} />}>
                    복사해서 시작
                  </Button>
                </CardFooter>
              </Card>
        ...
        <Section title="최근 저장한 항목" actionLabel="내 저장소">
                  ...
                  <div className="flex-1">
                    <div className="mb-2 flex flex-wrap gap-2">
                      {item.badges.map((badge) => (
                        <Badge key={badge} variant={badge === '입문' ? 'green' : 'blue'}>
                          {badge}
                        </Badge>
                      ))}
                    </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  블록이 무엇인지, 흐름을 어떻게 만드는지 빠르게 살펴보세요.
                </p>
              </div>
            </div>

            <Button variant="secondary">안내 가이드 보기</Button>
          </CardBody>
        </Card>
      </PageContainer>
